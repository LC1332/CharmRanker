"""
Dynamic OpenAI - 使用 GPT-5.2-chat 进行动态图片颜值比较
...
from src.triplet2message.triplet2message import (
    ATTRACTIVENESS_PROMPT, 
...
class DynamicOpenAI:
    ...
    def _build_openai_message(self, image_a: Path, image_b: Path, image_c: Path) -> dict:
        """构建 OpenAI 消息格式"""
        content = [
    ...
    def _call_openai_api(self, message: dict) -> str:
        """调用 OpenAI API"""
        if not LUMOS_API:
        ...
        # gpt-5.2-chat 只支持 temperature=1
        temperature = self.temperature
        if "gpt-5.2-chat" in self.model:
            temperature = 1.0
    ...
    def _worker_task(self, triplet_info: Dict) -> Tuple[Dict, Optional[Dict], str]:
        ...
        try:
            # 构建消息
            message = self._build_openai_message(*image_paths)
            
            # 调用 API
            raw_response = self._call_openai_api(message)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Dynamic OpenAI 颜值比较")
    parser.add_argument("--config", "-c", default=None,
                        help="配置文件路径 (默认: src/dynamic_openai/config.yaml)")
    ...
    if not OPENAI_BASE_URL:
        print("错误: 请在 .env 文件中设置 OPENAI_BASE_URL")
        sys.exit(1)
    
    # 运行
    dynamic_openai = DynamicOpenAI(config_path=args.config)
    dynamic_openai.run()
